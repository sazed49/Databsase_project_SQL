...
-- FIND POPULATION NAD HOSPITAL OF PLACES OF FREEDOMFIGHTER
SELECT DIS.POPULATION,DIS.HOSPITAL
FROM DISTRIBUTIONS DIS
WHERE DIS.AREA_ZIP_CODE IN(
	SELECT P.AREA_ZIP_CODE
	FROM PEOPLE P
	WHERE P.FREEDOMFIGHTER='Y'
);
--FIND ELIGIBLE PEOPLE(DOCTOR,40AGE,GOVT SERVICE) AND THEIR CORRESPONDIND VACCINE
SELECT VAC.MANUFACTURER
FROM VACCINE_TABLE VAC NATURAL JOIN DISTRIBUTOR_TABLE DIS
WHERE DIS.AREA_ZIP_CODE IN(
	SELECT P.AREA_ZIP_CODE
	FROM PEOPLE P
	WHERE P.AGE>40 OR P.PROFESSION='DOCTOR' OR P.PROFESSION='GOVT.SERVICE'
	);

-- FIND LIST OF PEOPLE UNDER 40 AND GENDER USING UNION
SELECT P.NAME
FROM PEOPLE P
...
-- FIND NAME OF PEOPLE WHO IS FREEDOM FIGHTEWR AND HAVE DIABETES(INTERSECT)
(SELECT P.NAME 
FROM PEOPLE P 
WHERE P.FREEDOMFIGHTER='Y')
INTERSECT
(SELECT M.NAME 
FROM MEDICAL_INFO M 
WHERE M.PREVIOUS_DIFFICULTIES='DIABETES');

-- PL SQL TO FIND MOST AGED VACCIE RECEIVER
SET SERVEROUTPUT ON
DECLARE
...
FROM MEDICAL_INFO;
DBMS_OUTPUT.PUT_LINE('MOST SENIOR RECEIVER IS: '|| MOST_AGED_VACCINE_RECEIVER);
END;
/
--PL SQL TO FIND TOTAL POPULATION OF AREAS WHERE VACCINATION IS GOING TO BE HELD
SET SERVEROUTPUT ON
DECLARE
...
END;
/
--PL SQL AND IF ELSE TO FIND AGE AND VERDICT OF PERSON'S NAME TAKEN AS INPUT
SET SERVEROUTPUT ON
DECLARE
...
END;
/
-- using PLSQL and simple loop to find area code,total population and hospital
SET SERVEROUTPUT ON
DECLARE
CURSOR DISTRIBUTION_CUR IS
SELECT AREA_ZIP_CODE,POPULATION,HOSPITAL FROM DISTRIBUTIONS;
...
BEGIN
 ...
 DBMS_OUTPUT.PUT_LINE('AREA CODE: '||DIS_RECORD.AREA_ZIP_CODE||' TOTAL POPULATION: '||DIS_RECORD.POPULATION||' HOSPITAL: '||DIS_RECORD.HOSPITAL);
 END LOOP;
...
BEGIN
 ...
 END;
 /
 -- USING PROCEDURE TO FIND AREA CODE OF NAME TAKEN AS INPUT
 SET SERVEROUTPUT ON
 CREATE OR REPLACE PROCEDURE GETNAME IS
 PATIENT_NAME MEDICAL_INFO.NAME%TYPE;
 AREA_CODE MEDICAL_INFO.AREA_ZIP_CODE%TYPE;
 BEGIN
 PATIENT_NAME := '&PATIENT_NAME';
 SELECT MEDICAL_INFO.AREA_ZIP_CODE INTO AREA_CODE
 FROM MEDICAL_INFO
 WHERE MEDICAL_INFO.NAME=PATIENT_NAME;
 DBMS_OUTPUT.PUT_LINE('NAME: '|| PATIENT_NAME||' AREA CODE: '||AREA_CODE);
 END;
 /
 BEGIN
 GETNAME;
...
--FIND AVERAGE AGE OF ALL PEOPLE
 CREATE OR REPLACE FUNCTION avg_AGE RETURN NUMBER IS
   avg_AGE MEDICAL_INFO.AGE%type;
BEGIN
...
END TR_id;
/
INSERT INTO PEOPLE VALUES (NULL, 'SAZED', '17', 'Student', 'N', 1211);
SELECT * FROM PEOPLE;
